...
Here we will look at some basic statistics and visualizations for the New and Old Testament.
We will look again at number of verses, chapters and book, but additionally look also at the text itself.
For completeness we also include the statistics for the the whole Bible for completeness.
To maintain consistency we hence also printed all the statistics for the whole Bible even though some of them are repeated from the previous sections.

Since we are going to do some word-based analysis let's clean the text a bit by removing punctuation and converting everything to lowercase.
...
def average_words_per_verse(df, testament_name):
    df['word_count'] = df['cleaned_text'].apply(lambda x: len(x.split()))
    average_words = df['word_count'].mean()
    print(f"Average number of words per verse in {testament_name}: {average_words:.2f}")

def plot_top_words(df, testament_name, number_of_words=20):
    top_words = pd.Series(
        ' '.join(df['cleaned_text']).split()
...
average_words_per_verse(bible_df, "Bible")
```
...
## Plot
```{python}
top_words_bible_filtered = plot_top_words_filtered(bible_df, "Bible", stopwords=stopwords)
```
...
average_words_per_verse(old_testament_df, "Old Testament")
```
...
## Plot
```{python}
top_words_ot_filtered = plot_top_words_filtered(old_testament_df, "Old Testament", stopwords=stopwords)
```
...
average_words_per_verse(new_testament_df, "New Testament")
```
...
## Plot
```{python}
top_words_nt_filtered = plot_top_words_filtered(new_testament_df, "New Testament", stopwords=stopwords)
```
...
## Plot
```{python}
# barplot of number of verses per Testament
verse_counts = bible_df['testament'].value_counts()
plt.figure(figsize=(8, 8))
sns.barplot(x=verse_counts.index, y=verse_counts.values, palette="pastel")
plt.title("Number of verses per Testament")
...
## Data
```{python}
print("Number of verses per Testament:")
print(verse_counts)
```
:::
...
## Plot
```{python}
avg_words_per_verse = bible_df.groupby('testament')['cleaned_text'].apply(lambda x: x.str.split().apply(len).mean())
plt.figure(figsize=(8, 8))
sns.barplot(x=avg_words_per_verse.index, y=avg_words_per_verse.values, palette="pastel")
plt.title("Average number of words per verse per Testament")
plt.xlabel("Testament")
plt.ylabel("Average number of words per verse")
plt.show()
```
## Data
```{python}
print("Average number of words per verse per Testament:")
print(avg_words_per_verse)
```
:::
...
top_new = np.argsort(coef)[-20:]
top_old = np.argsort(coef)[:20]

top_features = np.concatenate([top_old, top_new])
top_values = coef[top_features]
top_labels = feature_names[top_features]

plt.figure(figsize=(8, 8))
positions = np.arange(len(top_features))
plt.barh(positions, top_values)
plt.yticks(positions, top_labels)
plt.title("Top 15 Discriminative Words (Old vs New Testament)")
...
# Convert labels to binary (Old = 0, New = 1)
y_bin = label_binarize(y_test, classes=["Old Testament", "New Testament"]).ravel()

idx = list(clf.classes_).index("New Testament")

# Predict probabilities for the New Testament
y_prob = clf.predict_proba(X_test_tfidf)[:, idx]
