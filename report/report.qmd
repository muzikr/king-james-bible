...
```{python}
print(f"Number of verses in Old Testament: {len(old_testament_df)}")
print(f"Number of books in Old Testament: {old_testament_df['book'].nunique()}")
print(f"Number of chapters in Old Testament: {old_testament_df.groupby(['book', 'chapter']).ngroups}")
```

```{python}
book_lengths = old_testament_df.groupby('book').size()
print(f"Longest book in Old Testament: {book_lengths.idxmax()} ({book_lengths.max()} verses)")
print(f"Shortest book in Old Testament: {book_lengths.idxmin()} ({book_lengths.min()} verses)")
```

```{python}
average_verses_per_book = book_lengths.mean()
print(f"Average number of verses per book in Old Testament: {average_verses_per_book:.2f}")
```

:::{.panel-tabset}
## Plot
```{python}
top_words_ot = pd.Series(
    ' '.join(old_testament_df['text']).lower().split()
).value_counts().head(20)

plt.figure(figsize=(12, 6))
sns.barplot(
    x=top_words_ot.index,
    y=top_words_ot.values,
    palette="viridis"
)
plt.xticks(rotation=90)
plt.title("Top 20 most common words in Old Testament")
plt.xlabel("Words")
...
```{python}
# Create wordcloud for Old Testament
wordcloud_ot = WordCloud(width=800, height=400, background_color='white').generate(' '.join(old_testament_df['text']).lower())
plt.figure(figsize=(15, 7.5))
plt.imshow(wordcloud_ot, interpolation='bilinear')
plt.axis('off')
plt.title("Word Cloud for Old Testament")
plt.show()
```
...
"""
TODO:
- average number of verses per book in each testament
"""
